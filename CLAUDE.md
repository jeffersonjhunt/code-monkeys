...
- **`bin/`**: Host shim scripts symlinked individually into `~/.local/bin/` — `primate-pull` (**the one** ECR pull: ensures `<image>:latest` is local, fetching a token from ~/.aws via the public `amazon/aws-cli` image and logging in under a throwaway `DOCKER_CONFIG`, so 12-hour tokens never reach the host's real config or keychain), `aws` (local-first wrapper that falls back to running in the `minion` container if no `aws` binary is on PATH), `sops`/`age`/`age-keygen` (run in the `nyckel` primate), `spark-bench` (runs an eval harness in the `spark-bench` primate; see `007/skills/spark-bench/`), and `zoo` (see below). Every one of these ensures its image through `primate-pull`, as do `vault` and `primate()` — that logic used to be copied into five files and missing from the two that needed it most, so a pruned host met "pull access denied … may require 'docker login'" and was sent to `make`
- **`bin/zoo`**: `top(1)` for primates — one python3 script, standard library only (`curses`, HTTP over the docker unix socket, `subprocess`), implementing `zoo-spec.md`. Lists every container carrying the `primate.*` labels (running or stopped, with live CPU/memory/pids), and on the selection: `a` attach/resume a session's tmux, `e` a new shell, `x` remove after a confirm, `n`/`s` start a primate/session from the roster. **zoo runs inside tmux, one instance per terminal**: launched outside tmux it re-execs itself into its OWN unique session (`tmux new-session -s zoo-<uuid>`, no `-A`, so terminals do not mirror), as window 0, and `a`/`e`/`n`/`s` open **new tmux windows** rather than handing over the terminal — so the list keeps refreshing and there is no hand-off code (the source of most of the earlier zsh version's bugs). `ctrl-b w`/`ctrl-b 0` switch and return; a `primate-session` nests tmux, so its prefix is doubled (`ctrl-b ctrl-b`). **tmux is a host prerequisite** — in every primate image, but a bare host needs it installed; without it `zoo` exits with a message (no fallback, by design). `--once` is the exception: a plain unwrapped frame, no tmux needed. Each launch is its own instance (its own list process and session); primates are shared via the docker daemon, not tmux windows. `q` returns to the shell — exits if only the list window is open, else detaches the client so launched/attached windows keep running; stale sessions zoo created (marked with a `@zoo` tmux option, detached, only the list window) are reaped on startup — a user's session merely named `zoo-…` is never touched. Seams add `ZOO_SESSION`. Refusing inside an existing tmux still holds (running in a foreign session is a possible later change). Design points that are not preferences: identification is by label only (never image name — `spark-bench` eval containers share the roster's image); every action re-inspects the selected **64-character id** at the moment of action, so a same-named recreated session is refused, not hit; the container zoo runs in (from `/containers/<id>/` in `/proc/self/mountinfo`, empty on a host) is marked `(here)` and never a target; new windows are targeted at the session with a trailing colon (`new-window -t zoo:`) because window 0 shares the session name; launch and attach call the **zsh functions** by sourcing `~/.zfuncs` in a child zsh, so `zfuncs` stays the one owner of mounts, guards and first-run sync; stats use the API's `one-shot=true` (~6 ms) and compute CPU% as a delta between ticks, so there is no sampler process, no coprocess, no stream. Reaches hosts via `setup`'s `bin/` symlink and home volumes via `upgrade-home.sh`; inside a primate the roster is empty (standalone `~/.zfuncs`, no `primates/`), so `n`/`s` are not offered there. Tests: `python3 -m unittest discover -s tests -p 'test_zoo*.py'` — a hermetic model suite; a pty suite that runs zoo from an interactive `zsh -f -i` (job control on) against a fake daemon and a stub tmux (asserting the tmux commands zoo issues); and a real-tmux end-to-end case against an isolated server (`TMUX_TMPDIR`) that caught a target bug the stub could not. Opt-in live suite `ZOO_LIVE=1` needs tmux/zsh/docker. Seams: `DOCKER_HOST`, `ZOO_ZFUNCS`, `ZOO_MOUNTINFO`, `ZOO_TMUX`, `ZOO_HOST_CAPS`, `ZOO_CONFIG`. **UX (v0.3.0):** rows coloured by state (fallback to plain attributes with no colour, so colour is never the sole signal); the title shows host cpu/RAM from `/info` and a Σ of the running primates; the launch picker greys and refuses images this host can't run (`IMAGE_REQUIREMENTS` + `host_capabilities`/`runnable`: cuda-* need an NVIDIA GPU, spark-bench amd64); the picker, name prompt and kill confirm are centered curses popup boxes; and a letter in the picker is type-ahead (`jump`). All the new decisions are pure functions with unit tests (`style_token`, `aggregate`, `host_facts`, `host_capabilities`, `runnable`, `jump`). **Settings (v0.5.0):** per-machine prefs persist in a plain `key = value` `~/.zoo` (`ZOO_CONFIG` overrides; a bad/unknown line warns and falls back to the field default, never blocks start; precedence is defaults < `~/.zoo` < `--interval`). `,` opens a centered settings screen (up/down move, left/right change, Enter/`w` save, Esc/`q` cancel) editing `interval`, the `header`/`selected` styles and the `running`/`stale`/`pending` colours; saving writes the file and applies live (interval + styles re-resolve, no restart). The **header now defaults to bold-underline and the selection to reverse** — before, both were reverse and the selected row read as a second header bar. Pure, unit-tested: `parse_config`/`render_config`, `resolve_interval`, `style_attr`/`curses_color`, `cycle_setting`.
...
- **`primates/upgrade-home.sh`**: The one home-volume dotfile-sync script, run inside a `codemonkey:latest` container against a `<image>-home` volume. Both `primate-upgrade` (in `zfuncs`) and `make <name>.upgrade` invoke it; it used to be duplicated inline in both, and the copies had drifted. It syncs the shell dotfiles, the claude config (`settings.json`, `CLAUDE.md`, `commands/`), and — last and guarded, like `primate-pull` and `zoo` — the **007 agent skills** by running `007/skills/scripts/install-skills.sh`, so a primate's `~/.claude/skills`/`~/.kiro/skills` get the repo's skills on first-run seed and on every upgrade. It runs the plain script, not `make` — code-monkeys Makefiles are host entry points and are never invoked inside a container; the Makefile's `install` target delegates to the same script, so host `setup` and the in-container sync share one definition. A missing or failed install warns but never aborts the rest of the sync.
...
- **`007/skills/`**: Agent skills library — portable skills installed into `~/.kiro/skills/` and `~/.claude/skills/` by `setup` on a host, and into a primate's home volume by `upgrade-home.sh` (so first-run seed and `make <name>.upgrade`/`primate-upgrade` both give primates the skills — `~/.claude` is not bind-mounted, only `~/.ssh`/`~/.aws` are). One definition of the install, `007/skills/scripts/install-skills.sh`: the Makefile's `install` target delegates to it (host `setup`), and `upgrade-home.sh` runs it directly (the in-container sync), so no Makefile is ever run inside a container. Run `make test` from `007/` to test. See `007/skills/CONTRIBUTING.md` for authoring guidelines.
