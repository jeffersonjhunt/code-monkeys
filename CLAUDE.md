...
- **`bin/zoo`**: `top(1)` for primates — one python3 script, standard library only (`curses`, HTTP over the docker unix socket, `subprocess`), implementing `zoo-spec.md`. Lists every container carrying the `primate.*` labels (running or stopped, with live CPU/memory/pids), and on the selection: `a` attach/resume a session's tmux, `e` a new shell, `x` remove after a confirm, `n`/`s` start a primate/session from the roster. Design points that are not preferences: identification is by label only (never image name — `spark-bench` eval containers share the roster's image); every action re-inspects the selected **64-character id** at the moment of action, so a same-named recreated session is refused, not hit; the container zoo runs in (from `/containers/<id>/` in `/proc/self/mountinfo`, empty on a host) is marked `(here)` and never a target; launch and attach call the **zsh functions** by sourcing `~/.zfuncs` in a child zsh, so `zfuncs` stays the one owner of mounts, guards and first-run sync; stats use the API's `one-shot=true` (~6 ms) and compute CPU% as a delta between ticks, so there is no sampler process, no coprocess, no stream. Reaches hosts via `setup`'s `bin/` symlink and home volumes via `upgrade-home.sh`; inside a primate the roster is empty (standalone `~/.zfuncs`, no `primates/`), so `n`/`s` are not offered there. Tests: `python3 -m unittest discover -s tests -p 'test_zoo*.py'` — a hermetic model suite plus a pty suite that runs zoo from an interactive `zsh -f -i` (job control on) against a fake daemon on a unix socket; asserts content against a VT100 screen model, escape sequences against the stream, and `stty -g` before/after. Seams: `DOCKER_HOST`, `ZOO_ZFUNCS`, `ZOO_MOUNTINFO`.
