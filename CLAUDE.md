...
- **`zfuncs`**: Shell functions for launching containers (`primate()`, `primate-session()`, `primate-session-list()`, `primate-session-kill()`, `primate-upgrade()`, `clamscan()`, etc.). `primate()` runs a foreground `--rm` container tied to the TTY; `primate-session()` runs a **detached, named, long-lived** container (PID 1 = `sleep infinity`) and `docker exec`s into an in-container `tmux` session, so the session survives SSH disconnects — reconnect and re-run `primate-session <image>` to re-attach. A second positional argument names the session (`primate-session claude scratch`), so one image can have several in flight at once. Session containers are **labelled** (`primate.session`, `primate.image`, `primate.name`) — `primate-session-list` shows what is in flight and `primate-session-kill <session>` tears one down (the `<image>-home` volume persists). Both resolve through the label, never a bare container name, so an unrelated container that shares a session's name can never be matched. Both `primate()` and `primate-session()` refuse to start from `$HOME` — the workspace mount is `$(pwd)`, so that would hand the container every dotfile read-write; pass `--allow-home` to mean it, or `--no-workspace` to skip the mount (`primate-session` re-attaching to an existing container makes no new mount and is never refused).
