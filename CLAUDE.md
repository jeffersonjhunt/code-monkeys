...
- **`primates/fleet.conf`**: **Single source of truth for the fleet roster.** One colon-delimited row per primate — `name:chain:in_all:arches:push_default:manifest_default:home_volume` — parsed with `awk -F:` from the `primates/Makefile`, `build-push.sh`, `manifest-push.sh` and `zfuncs`. Every list of "which primates exist" is *derived* from it by filtering on those fields; none is hand-maintained. **Row order is the build order** (`miniforge3` must precede everything that `FROM`s it) — do not alphabetize. Adding a primate is one row, not five edits. Every consumer fails loudly on a missing or malformed inventory rather than iterating an empty set. A copy is installed as `~/.fleet.conf` inside codemonkey-based images (baked by `codemonkey.dockerfile`, synced by `primates/upgrade-home.sh`) so the in-container `zfuncs` helpers can read it.
