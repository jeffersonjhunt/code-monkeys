...
- **`claude/`**: Claude Code settings and custom slash commands (copied into the claude primate image). **Note:** `setup`'s `DIR_LINKS` entry is `claude::claude`, and `create_dir_links()` links `$SRC/<name>` → `$HOME/.<parent>/<name>` — so it actually creates `~/.claude/claude` → `<repo>/claude`, **not** `~/.claude` → `<repo>/claude`. Claude Code therefore does *not* pick these settings up from `~/.claude/settings.json` on the host; `make -C primates <img>.upgrade` is what copies `settings.json` + `commands/` into a primate's `~/.claude/`. (Documented as-is — the intent was probably `~/.claude`, but changing `setup` would relocate every existing install's links.)
