...
- **`zfuncs`**: Shell functions for launching containers (`primate()`, `primate-session()`, `primate-session-list()`, `primate-session-resume()`, `primate-session-kill()`, `primate-upgrade()`, `zoo()`, `clamscan()`, etc.). `primate()` runs a foreground `--rm` container tied to the TTY; `primate-session()` runs a **detached, named, long-lived** container (PID 1 = `sleep infinity`) and `docker exec`s into an in-container `tmux` session, so the session survives SSH disconnects — reconnect and re-run `primate-session <image>` to re-attach. A second positional argument names the session (`primate-session claude scratch`), so one image can have several in flight at once. Session containers are **labelled** (`primate.session`, `primate.image`, `primate.name`) — `primate-session-list` shows what is in flight and `primate-session-kill <session>` tears one down (the `<image>-home` volume persists). Both resolve through the label, never a bare container name, so an unrelated container that shares a session's name can never be matched. `primate-session-resume <session>` re-attaches by session name alone, without having to remember the image, and with one session in flight the argument is optional; it refuses to attach the session you are already sitting in to itself. All three share one label-based resolver, so -kill and -resume can never disagree about which container a name means. **`zoo`** is top(1) for primates: a full-screen table of every primate and session with live CPU and memory, refreshed on a tick. `↑`/`↓` select a row and `k` kills it after a confirm; `j`/`k` are deliberately not bound to navigation, since `k` is kill and overloading it is how a navigation keystroke becomes a destructive one. Sessions are killed through `primate-session-kill` so the label resolver stays the single answer to which container a name means, and the `<image>-home` volume persists; a foreground primate is `--rm`, so removing it *is* the kill. zoo refuses to kill the container it is itself running in — the comparison is a prefix, because `docker ps` reports a short id and `_primate_self_container_id` a full one, and an equality test would leave the guard looking present while never matching. `a` re-attaches to a session through `primate-session-resume`, and refuses a foreground primate — it has no tmux server, its shell belonging to the terminal that started it. `e` opens a **new** shell in any running container and is deliberately not called attach, since it is not the original terminal. Both share `_zoo_target_ok` with the kill: a real row, not zoo's own container, and — for a session, which downstream code addresses by reusable name — still the container that was selected. `n` and `N` pick an image from the roster with the same ↑↓ idiom and hand over to `primate-session` / `primate`, which own the workspace mount, the `$HOME` refusal and the first-run sync — zoo picks a name and gets out of the way. The picker shows the directory that will be mounted before you commit to it, and inside a primate (where `~/.zfuncs` has no sibling `primates/`) it says there is no roster rather than offering an empty list. Every key that hands over the terminal returns to zoo when the child ends, and `q`, `esc` and ctrl-c all quit. Both the table and the picker **window to the terminal height**, following the selected row and marking `⋯ N more above/below`: a list taller than the screen scrolls the alternate screen, after which every `tput cup 0 0` lands in the wrong place — 17 roster images against a 24-line terminal was enough to do it. `--once` and a pipe are never windowed, and the header counts all rows rather than the visible ones. Each frame is emitted as a single write, because a print per line is a visible paint step per line. Containers are found by label (`primate.managed`, plus `primate.session` for sessions predating it), never by matching image names against the roster: on `intel-nuc` six real `spark-bench` eval containers come from an image that *is* in the roster, so name-matching would have listed all six as primates. The refresh is two-tier because `docker ps` costs ~0.03s and `docker stats` ~2.05s (a fixed sampling window, measured) — stats are sampled in a background job and the table renders from the last completed sample, marked stale past three ticks, so keystrokes never wait on them. `zoo --once`, or any non-TTY stdout, prints one frame and exits. **zsh completion** covers `primate`, `primate-session` (images, then live session names), `primate-session-kill`/`-resume` (live sessions) and `primate-upgrade` (only the primates that actually have a home volume, plus `--all`), with `--no-workspace`/`--allow-home` offered on both launchers. On the **first run of an image** (no `<image>-home` volume yet) both seed the volume from the image and then run `primates/upgrade-home.sh` against it, so the repo's real configs are in place before the shell starts — the images can only ship placeholders for the gitignored ones (`opencode.json` names `LITELLM_HOST`; the claude primate's `settings.json`/`commands` likewise), and nothing on the `primate` path used to replace them, so a clean install could not produce a working opencode. Seeding happens first because docker copies image content only into a volume that is empty at mount time. It never blocks the launch: no host repo checkout (inside a primate, say) warns and starts with image defaults. Both `primate()` and `primate-session()` refuse to start from `$HOME` — the workspace mount is `$(pwd)`, so that would hand the container every dotfile read-write; pass `--allow-home` to mean it, or `--no-workspace` to skip the mount (`primate-session` re-attaching to an existing container makes no new mount and is never refused).
