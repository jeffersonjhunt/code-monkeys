...
3. **No separate `/srv` mount** — `/srv/models` is a plain directory on the root partition (not its own volume), so the weights compete for root's space like anything else. Ample headroom for a ~50 GB NVFP4 model; check before a pull if it tightens. *(This note originally read "weights live under `~/Models`" — see `decisions.md` 2026-07-13: the deployed location is and remains `/srv/models`.)*
